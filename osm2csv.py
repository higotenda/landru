...
FILTER = r'["highway" ~ "' + '|'.join(set(FREE_FLOW_SPEED_MAP.keys()) - PRUNE_SET) + '"]';
...
def edge_proc(edges_iter):
    ...
    for i, edge in enumerate(edges_iter):
        ...
        defined_free_flow = float(FREE_FLOW_SPEED_MAP.get(rtype, DEFAULT_FREE_FLOW_SPEED));
        if FUDGE_IDEAL_VALUES:
            free_flow = random.gauss(defined_free_flow + (FREEFLOW_MEAN_SHIFTCOEF)*road_len, FREEFLOW_SPEED_VARIANCE);
        ...
        yield (name, edge[0], edge[1], road_len, free_flow, jd, 1, edge[2].get('name', 'unnamed'));
