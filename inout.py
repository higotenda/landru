...
def get_location_from_user():
    while True:
        user_location = input(
            "Enter the coordinates (latitude,longitude) of the crowded place: "
        )
        if "," in user_location and len(user_location.split(",")) == 2:
            return user_location
        else:
            print(
                "Invalid input format. Please enter the coordinates in the format 'latitude,longitude'."
            )
...
def main():
    if not API_KEY:
        print(
            "Error: Foursquare API key not found. Please set the FOURSQUARE_API_KEY environment variable."
        )
        return

    user_location = get_location_from_user()
    place_type = (
        input("Enter the type of place to search for (e.g., 'restaurant', 'cafe'): ")
        .strip()
        .lower()
    )
    ...
    if nearby_places:
        ...
        save_to_csv(crowded_place_coords, nearby_places_coords, "nearby_places.csv")

        print(
            f"The coordinates of the crowded place and nearby places have been saved to 'nearby_places.csv'"
        )
    else:
